...
exports.getTransactions = async (req, res) => {
  try {
    const { page = 1, limit = 10, userId, marketId, sortBy = "createdAt", sortOrder = "desc" } = req.query
...
exports.createTransaction = async (req, res) => {
  try {
    ...
    if (type === "BUY") {
      ...
      const userShares = outcome === "YES" ? position.sharesYes : position.sharesNo
    ...
    // Save all changes
    await Promise.all([transaction.save(), market.save(), position.save(), user.save()])

    await transaction.populate(["user", "market"])

    // Get updated market stats
    const marketStats = LMSRService.getMarketStats(market.lmsr.sharesYes, market.lmsr.sharesNo, market.lmsr.beta)
  ...
  try {
    ...
    const positionsWithPnL = await Promise.all(
      positions.map(async (position) => {
        ...
        const unrealizedPnL = currentValue - position.totalInvested + position.realizedPnL

        return {
          ...position.toObject(),
          currentValue: Math.round(currentValue * 100) / 100,
          unrealizedPnL: Math.round(unrealizedPnL * 100) / 100,
          totalPnL: Math.round((unrealizedPnL + position.realizedPnL) * 100) / 100,
        }
      }),
    )
...
exports.processEmailTransaction = async (req, res) => {
  try {
    ...
    if (!["BUY", "SELL"].includes(type)) {
      return res.status(400).json({ message: "Invalid command. Use BUY or SELL" })
    }
