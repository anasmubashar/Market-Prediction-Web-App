...
export const usersAPI = {
  async getUsers(page = 1, limit = 10, sortBy = "email", sortOrder = "asc"): Promise<UsersResponse> {
...
export const transactionsAPI = {
  async getTransactions(page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc"): Promise<TransactionsResponse> {
    const params = new URLSearchParams({
      page: page.toString(),
      limit: limit.toString(),
      sortBy,
...
export const emailsAPI = {
  async sendMarketCycle() {
    const response = await fetch(`${API_BASE_URL}/emails/send-cycle`, {
      method: "POST",
    })
